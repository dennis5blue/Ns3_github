...
YansWifiPhy::SetChannelNumber (uint16_t nch)
{
  ...
  switch (m_state->GetState ())
    {
    case YansWifiPhy::RX:
      NS_LOG_UNCOND ("drop packet because of channel switching while reception");
      m_endRxEvent.Cancel ();
      goto switchChannel;
      break;
    case YansWifiPhy::TX:
...
WifiMode txMode=txVector.GetMode();
  ...
  switch (m_state->GetState ())
    ...
    case YansWifiPhy::SWITCHING:
      NS_LOG_UNCOND ("drop packet because of channel switching");
      NotifyRxDrop (packet);
      /*
      ...
      break;
    case YansWifiPhy::RX:
      NS_LOG_UNCOND ("drop packet because already in Rx (power=" <<
                    rxPowerW << "W)");
      NotifyRxDrop (packet);
      if (endRx > Simulator::Now () + m_state->GetDelayUntilIdle ())
        {
      ...
      break;
    case YansWifiPhy::TX:
      NS_LOG_UNCOND ("drop packet because already in Tx (power=" <<
                    rxPowerW << "W)");
      NotifyRxDrop (packet);
      if (endRx > Simulator::Now () + m_state->GetDelayUntilIdle ())
        {
...
YansWifiPhy::EndReceive (Ptr<Packet> packet, Ptr<InterferenceHelper::Event> event)
{
  ...
  NS_LOG_UNCOND ("mode=" << (event->GetPayloadMode ().GetDataRate ()) <<
                ", snr=" << snrPer.snr << ", per=" << snrPer.per << ", size=" << packet->GetSize ());
  //print my data (SNR vs. PER)
  std::cout << snrPer.snr <<" ";
  std::cout << snrPer.per << std::endl;
