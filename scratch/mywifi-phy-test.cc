...
PsrExperiment::Input::Input ()
  : distance (5.0),
    ...
    //txMode ("DsssRate5_5Mbps"),
    txPowerLevel (0),
    packetSize (2304),
    nPackets (400),
    errorRateModelFlag (1) //default is YansErrorRateModel
{
}
...
PsrExperiment::Run (struct PsrExperiment::Input input)
{
  ...
  //Ptr<OkumuraHataPropagationLossModel> log = CreateObject<OkumuraHataPropagationLossModel> ();
  ...
  Ptr<ErrorRateModel> error2 = CreateObject<NistErrorRateModel> ();
  Ptr<ErrorRateModel> error3 = CreateObject<YansErrorRateModel> ();
  
  if (m_input.errorRateModelFlag=='1') {
    //std::cout << "Error rate model flag: " << m_input.errorRateModelFlag << " (Yans)" << std::endl;
    tx->SetErrorRateModel (error);
    rx->SetErrorRateModel (error);
  }
  else if (m_input.errorRateModelFlag=='2') {
    //std::cout << "Error rate model flag: " << m_input.errorRateModelFlag << " (Nist)" << std::endl;
    tx->SetErrorRateModel (error2);
    rx->SetErrorRateModel (error2);
  }
  else if (m_input.errorRateModelFlag=='3') {
    //std::cout << "Error rate model flag: " << m_input.errorRateModelFlag << " (Dsss)" << std::endl;
    tx->SetErrorRateModel (error3);
    rx->SetErrorRateModel (error3);
  }
  else {
    std::cout << "You must give the wrong flag" <<std::endl;
    std::cout << "1:Yans, 2:Nist, 3:Dsss (For Dsss case, need to change the TxMode@PsrExperiment::Input::Input ())" <<std::endl;
  }
  ...
  //monitor->SerializeToXmlFile("mywifi-phy-test.xml", true, true);
...
static void PrintPsr (int argc, char *argv[])
{
  ...
  cmd.AddValue ("ErrorRateModel", "Yans=1, Nist=2, Dsss=3", input.errorRateModelFlag);
  cmd.Parse (argc, argv);
  ...
  //std::cout << "Output received: " << psr << std::endl;
  psr /= input.nPackets; //psr means packet success rate
  //std::cout << "Successfully received ratio: " << psr << std::endl;
}
...
static void PrintPsrVsDistance (int argc, char *argv[])
{
  ...
  cmd.AddValue ("ErrorRateModel", "Yans=1, Nist=2, Dsss=3", input.errorRateModelFlag);
  cmd.Parse (argc, argv);
  for (input.distance = 1.0; input.distance < 165; input.distance += 1.0)
    {
